...
def main():    
    ...
    # visualize_augmentation(test_dataset)f

    ### 3. model build
    model_list = []
    
    # model1 = models.swin_t(weights='IMAGENET1K_V1')
    # model1.head = nn.Linear(in_features=768, out_features=2)
    # model1.to(device)

    model = models.swin_b(weights="IMAGENET1K_V1")
    model.head = torch.nn.Linear(in_features=1024, out_features=2)
    model.to(device)
    ...
    # model4.classifier[6] = nn.Linear(in_features= 4096, out_features= 2)
    # model4.to(device)
...
vgg16으로 5epoch 마친 스크립트입니다
---------------------------
epoch = 10 고정 

lr = 0.0001 고정 (?) (데이터 양이 적어서) 

batch_size는 알아서 64, 128, 256 cuda 상황에 맞게
