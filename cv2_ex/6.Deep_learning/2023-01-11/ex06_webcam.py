...
if cap.isOpened():
  while True:
    ...
    pil_img = Image.fromarray(frame)    # numpy array를 이미지로 바꿀때 사용
    input_img = data_transforms(pil_img).reshape([1,3,200,200]).to(device)
    out = model(input_img)
    softmax_result = F.softmax(out)     # softmax : label 별로 점수를 줘서 가장 높은것을 채택/ 이진분류에서도 사용됨!
    ...
    # label 이름 표시 
    cv2.putText(frame, labels.get(int(top1_label)), (10, 400), cv2.FONT_HERSHEY_DUPLEX, 2, (255, 255, 255), 1)  # (10, 400) text위치
